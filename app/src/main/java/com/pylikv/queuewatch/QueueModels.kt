...
/**
 * Статистика прохождения одного сегмента очереди.
 *
 * Очередь условно разделяется на 10 сегментов
 * по доле позиции относительно размера очереди:
 *
 * 0 = 0–10%
 * 1 = 10–20%
 * ...
 * 9 = 90–100%
 *
 * Эта статистика используется гибридным
 * прогнозом времени до вызова.
 */
data class QueueSegmentStatistics(
     ...
     * прохождения позиций внутри сегмента.
     *
     * В минутах.
     */
    ...
    /**
     * Количество фактически пройденных
     * позиций, по которым удалось
     * определить время.
     */
    val positionSamples: Int = 0

) {

    /**
     * Среднее время прохождения
     * одной позиции в этом сегменте.
     */
    val minutesPerPosition: Double?
        get() =
            if (
...
/**
 * Текущая скорость движения очереди.
 *
 * Это НЕ историческая скорость.
 *
 * Она рассчитывается по последним
 * фактическим изменениям очереди
 * и используется только как
 * корректирующий фактор прогноза.
 */
data class CurrentQueueSpeed(
