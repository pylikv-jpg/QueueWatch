...
class ForecastCoordinator(context: Context) {
    ...
    fun observe(
        ...
        val changedVisit = registration.isNotBlank() &&
            prefs.getString("registration", "").orEmpty().let { it.isNotBlank() && it != registration }

        if (
            prefs.getString("car_key", null) != key ||
            changedVisit ||
            (prefs.getBoolean("completed", false) && vehicle?.status == 2 && (vehicle.position ?: 0) > 0)
        ) {
            prefs.edit().clear()
                .putString("car_key", key)
                .putString("session_id", UUID.randomUUID().toString())
                .putLong("started", now)
                .putString("registration", registration)
                .commit()
        ...
        if (vehicle == null) {
            unavailable("Данные автомобиля временно отсутствуют", now)
            telemetry.flush()
            return
        }
        ...
        if (vehicle.status == 3) {
            ...
            if (!prefs.getBoolean("completed", false) && lastQueue > 0) {
                telemetry.enqueue(
                    baseEvent("actual_call", checkpoint, vehicle.vehicleType.name)
                        .put("called_at", iso(now))
                        .put("last_in_queue_at", iso(lastQueue))
                )
                prefs.edit().putBoolean("completed", true).commit()
            }
            unavailable("Вызов подтверждён сервером", now)
            telemetry.flush()
            return
        }
        ...
        if (vehicle.status != 2 || position == null || position <= 0) {
            unavailable("Ожидаем подтверждённую позицию в очереди", now)
            telemetry.flush()
            return
        }

        val liveVehicles = vehicles.filter {
            it.vehicleType == vehicle.vehicleType && it.status == 2 && (it.position ?: 0) > 0
        }

        estimator.observePosition(now, position)
        prefs.edit().putLong("last_in_queue", now).apply()

        // Baselines are always checkpoint + vehicle type specific. CAR and TRUCK
        // never share movement speed or movement-cycle priors.
        val history = baselines.find(checkpoint, vehicle.vehicleType.name, now)
        val live = estimator.estimate(
            now = now,
            virtualWindowMinutes = history?.movementCycleMinutes ?: 40.0,
            minimumObservedMinutes = if (history == null) 30.0 else 10.0
        )

        val result = if (estimator.stationary(now)) {
            ForecastResult.Unavailable("queue_stationary")
        } else {
            ForecastEngineV1.estimate(
                ForecastInput(position, liveVehicles.size, history, live, now)
            )
        }
        ...
        if (result is ForecastResult.Available) {
            ...
            val changed =
                position != prefs.getInt("sent_position", -1) ||
                    kotlin.math.abs(previousEta - result.etaMinutes) >= 10 ||
                    prefs.getString("sent_confidence", "") != result.confidence.name

            prefs.edit()
                .putBoolean("available", true)
                .putFloat("eta", result.etaMinutes.toFloat())
                .putFloat("low", result.lowMinutes.toFloat())
                .putFloat("high", result.highMinutes.toFloat())
                .putString("confidence", result.confidence.name)
                .putFloat("speed", result.effectivePositionsPerHour.toFloat())
            ...
            if (telemetry.enabled && (since >= 900_000 || (since >= 60_000 && changed))) {
                telemetry.enqueue(
                    baseEvent("prediction", checkpoint, vehicle.vehicleType.name)
                        .put("observed_at", iso(now))
                        .put("current_position", position)
                        .put("queue_count_same_type", liveVehicles.size)
                        .put("historical_speed", history?.positionsPerHour ?: JSONObject.NULL)
                        ...
                        .put("historical_sample_count", history?.sampleCount ?: 0)
                        .put("data_gap", estimator.dataGap)
                        .put("stale_live_data", live == null)
                )
                prefs.edit()
                    .putLong("sent_at", now)
                    .putInt("sent_position", position)
                    .putFloat("sent_eta", result.etaMinutes.toFloat())
                    .putString("sent_confidence", result.confidence.name)
                    .apply()
            }
        } else {
            unavailable(
                if (estimator.stationary(now)) {
                    "Продвижения пока нет — время вызова неизвестно"
                } else {
                    "Недостаточно данных для прогноза. Наблюдаем движение очереди…"
                },
                now
            )
        }
    ...
    private fun baseEvent(type: String, checkpoint: String, vehicleType: String) = JSONObject()
        ...
        .put("event_type", type)
        .put("install_id", telemetry.installId)
        .put("forecast_session_id", prefs.getString("session_id", ""))
        .put("algorithm_version", ForecastEngineV1.VERSION)
        .put("checkpoint_id", checkpoint)
        .put("vehicle_type", vehicleType)
        .put("started_at", iso(prefs.getLong("started", 0)))
