...
For Play release, product flavors change the Gradle task and output path. Replace the old `bundleRelease` step with:

```yaml
      - name: Build signed Google Play stable AAB
        if: ${{ env.PLAY_KEYSTORE_B64 != '' }}
        env:
          QUEUEWATCH_KEYSTORE_PATH: ${{ runner.temp }}/queuewatch-upload.jks
          QUEUEWATCH_KEYSTORE_PASSWORD: ${{ env.PLAY_KEYSTORE_PASSWORD }}
          QUEUEWATCH_KEY_ALIAS: ${{ env.PLAY_KEY_ALIAS }}
          QUEUEWATCH_KEY_PASSWORD: ${{ env.PLAY_KEY_PASSWORD }}
        run: gradle bundleStableRelease

      - name: Upload Google Play stable AAB
        if: ${{ env.PLAY_KEYSTORE_B64 != '' }}
        uses: actions/upload-artifact@v4
        with:
          name: QueueWatch-GooglePlay-AAB
          path: app/build/outputs/bundle/stableRelease/app-stable-release.aab
```

Do not upload the Forecast flavor to the existing production application ID.
...
- Create: `app/src/main/java/com/pylikv/queuewatch/forecast/HistoricalBaselineCatalog.kt`
- Create: `app/src/main/java/com/pylikv/queuewatch/forecast/HistoricalBaselineRepository.kt`
- Create: `app/src/test/java/com/pylikv/queuewatch/forecast/HistoricalBaselineCatalogTest.kt`

**Interfaces:**
- Consumes: compact artifact generated by the QueueLoggerData backtest/baseline plan.
- Produces a pure Kotlin `HistoricalBaselineCatalog` with:
...
- `HistoricalBaselineRepository` is the thin Android asset loader that parses JSON and creates the catalog.
- Unit tests exercise the pure catalog, avoiding Android `org.json` stubs in local JVM tests.
...
- [ ] **Step 1: Write pure catalog tests**
...
- [ ] **Step 2: Implement pure catalog and Android asset loader**

`HistoricalBaselineCatalog` owns lookup/fallback logic with ordinary Kotlin data classes.

`HistoricalBaselineRepository` uses `org.json.JSONObject` only in Android runtime code to read `forecast_baselines_v1.json`, then constructs the pure catalog.
...
```bash
gradle testForecastDebugUnitTest --tests "com.pylikv.queuewatch.forecast.HistoricalBaselineCatalogTest"
```
...
git add app/src/main/assets/forecast_baselines_v1.json \
        app/src/main/java/com/pylikv/queuewatch/forecast/HistoricalBaselineCatalog.kt \
        app/src/main/java/com/pylikv/queuewatch/forecast/HistoricalBaselineRepository.kt \
        app/src/test/java/com/pylikv/queuewatch/forecast/HistoricalBaselineCatalogTest.kt
